...
Welcome! This document exercises everything Remarked can render so far. Move your caret **into** any element to see its raw markdown melt back in — move away and it renders again. Press `⌥⌘E` (Mac) or `Ctrl+Shift+Alt+E` to flip to the plain source editor and back.
...
- **Select some text** and click **B** (bold), *I* (italic), ~~S~~ (strikethrough), or `` ` `` (inline code) — the markers appear around your selection.
- **Put your caret on a plain line** and click **H1**, **H2**, or **H3** — the heading prefix appears and the line renders as a heading. Click again to remove it.
...
This line has **bold text**, *italics*, ~~strikethrough~~, and `inline code`. Click inside any of them — the markers reappear, dimmed, exactly where you left them. Try **⌘B**, *⌘I*, and `⌘\`` on a selection, and ⌘K to insert a [link](https://github.com/phyrdaus/remarked).
...
- Press **Enter** at the end of this line — the list continues by itself
...
`</>` (also **⌘/**) to edit the raw pipes — move the caret away and it snaps
back into a table.
...
it lands in `assets/` next to this file (configurable via
